...
class CTMCGrid(SpatialGrid):
    ...
    def _(self, coordinate: CoordinateND) -> tuple[float]:
        ...
        return tuple(
            self.axes[k][min(grid_length - 1, c + 1)] for k, c in enumerate(coordinate)
        )
...
class CTMCUniformGrid(CTMCGrid):
    ...
    def __init__(
        self,
        h,
        model: Union[LevyModel, LevyCopulaModel],
        truncation_probability=0.99999,
    ):
        ...
        """
        l, r = compute_truncation(
            model=model, h=h, truncation_probability=truncation_probability
        )
        nb_of_points_left = int(abs(l) / h)
        nb_of_points_right = int(r / h)
        if nb_of_points_left + nb_of_points_right > 1e8:
            raise ValueError(
                "the number of points is greater than 10M, choose a smaller value for the "
        ...
        super().__init__(
            h=h, origin_coordinate=pivot_position, axes=[axis] * model.dimension_model()
        )

    @classmethod
    def create_from_fixed_nb_of_points(
        cls, h: float, nb_of_points: int, dimension: int = 1
    ):
        ...
        super(CTMCUniformGrid, obj).__init__(
            h=h, origin_coordinate=pivot_position, axes=[axis] * dimension
        )
        return obj
...
class CTMCGridProbabilityStep(CTMCGrid):
    ...
    def __init__(
        ...
        h: float,
        model: LevyModel,
        minimum_probability_step: float = 0.05,
        dimension: int = 1,
    ):
        ...
        self.intensity_of_jumps = model.mass(-np.inf, -h / 2) + model.mass(
            h / 2, np.inf
        )
    ...
    def middle(self, xi: float, xip: float) -> float:
        ...
        sol_right = scipy.optimize.root_scalar(
            to_call, bracket=[xi, xip], method="brentq", x0=0.5 * (xi + xip), xtol=1e-10
        )
        sol = sol_right.root
...
class CTMCGridGeometric(CTMCGrid):
    ...
    def __init__(
        self,
        h: float,
        model: Union[LevyModel, LevyCopulaModel, LevyDrivenSDEModel],
    ...
    @classmethod
    def create_with_bounds(
        cls,
        h: float,
        truncations: tuple[float, float],
        dimension: int,
        nb_of_points_on_each_side: int = 2,
    ):
        ...
        obj = cls.__new__(cls)
        super(CTMCGridGeometric, obj).__init__(
            h=h, origin_coordinate=pivot_position, axes=[axis] * dimension
        )
        return obj
...
class CTMCCredit(CTMCGrid):
    ...
    def __init__(
        self,
        h: float,
        level_a: Union[float, list[float]],
        model: Union[LevyModel, LevyCopulaModel],
        symmetric_grid: bool = True,
    ):
        l, r = compute_truncation(model=model, h=h)
        if (
            isinstance(level_a, float)
            and level_a < l
            or isinstance(level_a, list)
            and any(a < l for a in level_a)
        ):
...
def compute_truncation(
    ...
    if model.dimension_model() == 1:
        ...
        l, r = compute_truncation_helper(
            levy_measure=levy_measure, h=h, probability=truncation_probability
        )
    else:
        models = (
            model.driver.models
            if isinstance(model, LevyDrivenSDEModel)
            else model.models
        )
        l_r = [
            compute_truncation_helper(
                levy_measure=m.levy_triplet.nu, h=h, probability=truncation_probability
            )
            for m in models
        ]
...
def compute_truncation_helper(
    levy_measure: LevyMeasure, h: float, probability: float
) -> tuple[float, float]:
    ...
    """
    grid = CTMCGrid(
        h=h, origin_coordinate=2, axes=[np.array([-3 * h, -h, 0, h, 3 * h])]
    )
    ...
    sol_right = scipy.optimize.root_scalar(
        fun_right, bracket=[h_right, 100], method="brentq"
    )
    sol_left = scipy.optimize.root_scalar(
        fun_left, bracket=[-100, h_left], method="brentq"
    )

    return sol_left.root, sol_right.root


def compute_right_axis(
    h: float, levy_measure: LevyMeasure, minimum_probability_step: float
):
    intensity_of_jumps = levy_measure.integrate(
        -np.inf, -h / 2
    ) + levy_measure.integrate(h / 2, np.inf)
    ...
    while True:
        p_left = (
            intensity_of_right_jumps - levy_measure.integrate(h / 2, middle_point)
        ) / intensity_of_jumps
        if p_left < minimum_probability_step / 2:
            ...
            break
        try:
            sol_middle = scipy.optimize.root_scalar(
                fun_right(start_right, minimum_probability_step / 2),
                bracket=[start_right, 100],
                method="brentq",
                xtol=1e-10,
            )
            middle_point = sol_middle.root
            sol_right = scipy.optimize.root_scalar(
                fun_right(middle_point, minimum_probability_step / 2),
                bracket=[middle_point, 100],
                method="brentq",
                xtol=1e-10,
            )
...
def compute_left_axis(
    ...
    while True:
        p_left = (
            intensity_of_left_jumps - levy_measure.integrate(middle_point, -h / 2)
        ) / intensity_of_jumps
        if p_left < minimum_probability_step / 2:
        ...
        try:
            sol_middle = scipy.optimize.root_scalar(
                fun_left(start_left, minimum_probability_step / 2),
                bracket=[-100, start_left],
                method="brentq",
                xtol=1e-10,
            )
            middle_point = sol_middle.root
            sol_left = scipy.optimize.root_scalar(
                fun_left(middle_point, minimum_probability_step / 2),
                bracket=[-100, middle_point],
                method="brentq",
                xtol=1e-10,
            )
