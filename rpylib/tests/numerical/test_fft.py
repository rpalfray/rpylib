...
def bs_model():
    model = create_exponential_of_levy_model(ModelType.BLACKSCHOLES)(
        spot=100.0, r=0.03, d=0.01, sigma=0.30
    )
    return model
...
def cgmy_model():
    ...
    model = create_exponential_of_levy_model(ModelType.CGMY)(
        spot=100.0, r=0.03, d=0.01, c=1.0, g=5.0, m=10.0, y=0.5
    )
    return model
...
def cgmy_model_II():
    ...
    model = create_exponential_of_levy_model(ModelType.CGMY)(
        spot=90.0, r=0.06, d=0.00, c=16.97, g=7.08, m=29.97, y=0.6442
    )
    return model
...
def cgmy_model_III():
    ...
    # by Forsyth, P. A., J. W. Wan, and I. R. Wang r
    model = create_exponential_of_levy_model(ModelType.CGMY)(
        spot=90.0, r=0.06, d=0.00, c=0.42, g=4.37, m=191.2, y=1.0102
    )
    return model
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            [0.8698, 2.2475, 5.8919, 2.1420, 0.7848],
        ),
        (
            0.50,
            [80.0, 90.0, 100.0, 110.0, 120.0],
            [1.8854, 4.0589, 8.8226, 4.7026, 2.3585],
        ),
        (
            0.75,
            [80.0, 90.0, 100.0, 110.0, 120.0],
            [2.8638, 5.5219, 11.0672, 6.8875, 4.1038],
        ),
        (
            1.00,
            [80.0, 90.0, 100.0, 110.0, 120.0],
            [3.7681, 6.7515, 12.9545, 8.7819, 5.7803],
        ),
    ],
    ids=[
        "T=0.25",
...
def test_fft_cgmy(cgmy_model, maturity, benchmark, strikes):
    ...
    assert all(
        abs(fft_price - cf_price) < threshold
        for fft_price, cf_price in zip(fft_prices, benchmark)
    )
