...
def test_cos_method_cgmy_from_fang_osterlee(y, expected):
    ...
    model = create_exponential_of_levy_model(ModelType.CGMY)(
        spot=spot, r=0.1, d=0.0, c=1.0, g=5.0, m=5.0, y=y
    )
...
def test_cos_method_variance_gamma_from_fang_osterlee(maturity, expected):
    ...
    model = create_exponential_of_levy_model(ModelType.VG)(
        spot=spot, r=0.1, d=0.0, nu=0.2, sigma=0.12, theta=-0.14
    )
    cos_pricer = COSPricer(model, n)
...
def test_cos_method_consistency_variance_gamma_cgmy():
    n = 2**12
    spot, r, d = 100.0, 0.1, 0.0
    nu, sigma, theta = 0.2, 0.12, -0.14
    eta_p = np.sqrt(theta**2 * nu**2 / 4 + sigma**2 * nu / 2) + theta * nu / 2
    eta_m = eta_p - theta * nu
    vg_model = create_exponential_of_levy_model(ModelType.VG)(
        spot=spot, r=r, d=d, nu=nu, sigma=sigma, theta=theta
    )
    cgmy_model = create_exponential_of_levy_model(ModelType.CGMY)(
        spot=spot, r=r, d=d, c=1 / nu, g=1 / eta_m, m=1 / eta_p, y=0
    )
