...
from .pairing import PairingToZd, Szudzik, RosenbergStrong, StatesManager, Domain, Boundary, PairingToZ1d
...
from .variate.binarysearchtreeadapted import BinarySearchTreeAdapted, BinarySearchTreeAdapted1D
...
def compute_intensity_of_jumps(model: Union[LevyModel, LevyCopulaModel], grid: CTMCGrid):
    ...
    intervals = [[[h_l, h_r], [grid.axes[k][0], h_l], [h_r, grid.axes[k][-1]]]
...
def create_sampling_inversion_method(grid, model, intensity_of_jumps, is_levy_copula):
    if is_levy_copula:
        if model.dimension() == 2:
            ...
            pairing = PairingToZd(pairing=RosenbergStrong(), dimension=model.dimension())
    else:
    ...
    return InversionMethod(probability_to_jump_to_state=probability_to_jump_to_state, state_manager=state_manager)
