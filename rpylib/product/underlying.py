...
class Underlying(abc.ABC):
    ...
    def _value_log(self, times, path: np.array, jump_path: np.array, payoff_underlying=None) -> np.array:
        ...
        raise NotImplementedError('Process log-representation not implemented for underlying '
...
class Asian(Underlying):
    ...
    def compute_times_grid(self, maturity: float) -> TimeGrid:
        ...
        if num < 2:
            raise ValueError('The maturity of the product is too small and inconsistent with the underlying')
...
class Performances(Underlying):
    ...
    def value(self, times, path: np.array, jump_path: np.array, payoff_underlying=None) -> float:
...
class NthSpot(Underlying):
    ...
    def __init__(self, index: int):
        ...
        self.index = index
        if index == 0:
            raise ValueError('expected index > 0, index=k means this is the k-th underlying spot')
...
class DefaultTimeNthUnderlying(_DefaultTimes):
    ...
    def value(self, times, path: np.array, jump_path: np.array, payoff_underlying=None) -> np.array:
        log_jump_ratio = np.diff(np.log(jump_path[self._k, ...]))  # consider just the k-th underlying
        default_time = np.inf
        idx = np.argwhere(log_jump_ratio < self._a)
        if idx.size > 0:
            default_time = times[np.min(idx) + 1]
    ...
    def _value_log(self, times, path: np.array, jump_path: np.array, payoff_underlying=None) -> np.array:
        log_jump_ratio = np.diff(jump_path[self._k, ...])  # consider just the k-th underlying
        default_time = np.inf
        idx = np.argwhere(log_jump_ratio < self._a)
        if idx.size > 0:
            default_time = times[np.min(idx) + 1]
...
class NthDefaultTimes(_DefaultTimes):
    ...
    def __init__(self, default_levels: list[float], index: int):
        ...
        super().__init__(default_levels=default_levels)
        if index == 0:
            raise ValueError('expected index > 0, index=k means this is the k-th default times')
        self._k = index - 1
