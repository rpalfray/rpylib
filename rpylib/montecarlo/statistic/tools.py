...
def stddev(simulations: np.array) -> np.array:
    if simulations.shape[0] == 1:  # only one piece of data hence no stddev
        return 0.0
    return np.std(simulations, axis=0, ddof=1)  # ddof=1 to have an unbiased estimator of the variance
...
class NonCenteredMoments:
    ...
    def ncm_second(self):
        ...
        m2_c = np.array([scipy.stats.moment(samples, moment=2) for samples in self.simulations])
        m2_nc = m2_c + mu**2
    ...
    def ncm_third(self):
        ...
        m3_c = np.array([scipy.stats.moment(samples, moment=3) for samples in self.simulations])
    ...
    def ncm_fourth(self):
        ...
        m4_c = np.array([scipy.stats.moment(samples, moment=4) for samples in self.simulations])
