...
class Statistic:
    ...
    def __init__(
        ...
        shape: tuple,
        mc_paths: int,
        process_representation: ProcessRepresentation,
    ):
        """
        ...
        self.density_plot = Density(
            label=label, process_representation=process_representation
        )
...
class MCStatistics:
    ...
    def __init__(
        self,
        *,
        payoff_statistics: Statistic,
        spot_underlying_statistics: Statistic = NoStatistic(),
        ...
        self._control_variates_statistics = (
            control_variates_statistics  # ->statistics of the control variates
        )
        self._payoff_statistics_with_cv = copy.deepcopy(
            payoff_statistics
        )  # -> statistics of the adjusted payoff
    ...
    def add(self, simulation: int, path_manager: "MCPath") -> None:
        ...
        self._control_variates_statistics.add(
            simulation, path_manager.payoff_control_variates
        )
    ...
    def _get_payoff_statistics(self, no_control_variates: bool = False) -> Statistic:
        if no_control_variates or isinstance(
            self._control_variates_statistics, NoStatistic
        ):
...
class MLMCResults:
    ...
    def __init__(
        self, Nl: np.array, sum_cost: np.array, all_pl_fine: list, all_pl_coarse: list
    ):
        """
        ...
        dp = [
            pl_fine - pl_coarse
            for pl_fine, pl_coarse in zip(all_pl_fine, all_pl_coarse)
        ]
    ...
    def kurtosis(self):
        ...
        val = (nsum4 - 4 * nsum3 * nsum1 + 6 * nsum2 * nsum1**2 - 3 * nsum1**4) / (
            np.maximum(1, nsum2 - nsum1**2)
        ) ** 2
        return val
    ...
    def vl(self):
        ...
        val = np.maximum(0.0, self._ncms_dp.ncm_second - self._ncms_dp.ncm_first**2)
        return val
...
class MLMCStatistics:
    ...
    def set_mlmc_results(self, Nl: np.array, sum_cost: np.array):
        all_pl_fine = [
            self.simulation_payoff_with_fine_process(level=level)
            for level in range(len(Nl))
        ]
        all_pl_coarse = [
            self.simulation_payoff_with_coarse_process(level=level)
            for level in range(len(Nl))
        ]
        self.mlmc_results = MLMCResults(
            Nl=Nl,
            sum_cost=sum_cost,
            all_pl_fine=all_pl_fine,
            all_pl_coarse=all_pl_coarse,
        )
    ...
    def _get_payoff_statistics(
        self, level: int, no_control_variates: bool = False
    ) -> Statistic:
        stats_level_l = self.mc_statistics[level]
        if no_control_variates or isinstance(
            stats_level_l._control_variates_statistics, NoStatistic
        ):
    ...
    def price(self, no_control_variates: bool = False) -> float:
        res = 0
        for level, mc_stat in enumerate(self.mc_statistics):
            stats_obj = self._get_payoff_statistics(
                level=level, no_control_variates=no_control_variates
            )
    ...
    def mc_stddev(self, no_control_variates: bool = False) -> float:
        res = 0
        for level, mc_stat in enumerate(self.mc_statistics):
            pl_fine = self.simulation_payoff_with_fine_process(
                level=level, no_control_variates=no_control_variates
            )
            pl_coarse = self.simulation_payoff_with_coarse_process(
                level=level, no_control_variates=no_control_variates
            )
    ...
    def simulation_payoff_with_fine_process(
        self,
        level,
        start: int = None,
        end: int = None,
        no_control_variates: bool = False,
    ) -> np.array:
        stats = self._get_payoff_statistics(
            level=level, no_control_variates=no_control_variates
        ).stats
        return stats[(slice(start, end), 0, PT.FP)]

    def simulation_payoff_with_coarse_process(
        self,
        level,
        start: int = None,
        end: int = None,
...
def create_mc_statistics(
    mc_paths: int,
    underlying_density: [Callable[[float], float]],
    control_variates: "ControlVariates",
    payoff_dimension: int,
    process_representation: ProcessRepresentation,
    activate_spot_statistics: bool,
    spot_dimension: int,
) -> MCStatistics:
    ...
    if isinstance(control_variates, NoControlVariates):
        ...
        control_variates_statistics = Statistic(
            "CV",
            shape=(len(control_variates.products), payoff_dimension),
            mc_paths=mc_paths,
            process_representation=process_representation,
        )

    return MCStatistics(
        payoff_statistics=payoff_statistics,
        spot_underlying_statistics=spot_underlying_statistics,
        control_variates_statistics=control_variates_statistics,
    )


def create_mlmc_statistics(
    mc_paths: int,
    underlying_density: [Callable[[float], float]],
    initial_level: int,
    control_variates: "ControlVariates",
    ...
    payoff_stat = Statistic(
        "payoff",
        shape=(payoff_dimension, 2),
        mc_paths=mc_paths,
        process_representation=process_representation,
    ...
    if underlying_density is not None:
        ...
        spot_underlying_stat = Statistic(
            "spot",
            shape=(spot_dimension,),
            mc_paths=mc_paths,
            process_representation=process_representation,
    ...
    if isinstance(control_variates, NoControlVariates):
        ...
        control_variate_statistics0 = Statistic(
            "CV",
            shape=(len(control_variates.products), payoff_dimension),
            mc_paths=mc_paths,
            process_representation=process_representation,
        )
    mc_stat0 = MCStatistics(
        payoff_statistics=payoff_stat,
        spot_underlying_statistics=spot_underlying_stat,
    ...
    def create_statistic(nb_of_paths=0):
        ...
        c_payoff_stat = Statistic(
            "payoff",
            shape=(payoff_dimension, 2),
            mc_paths=nb_of_paths,
            process_representation=process_representation,
        )

        c_spot_underlying_stat = NoStatistic()
        if underlying_density is not None:
            c_spot_underlying_stat = Statistic(
                "spot",
                shape=(spot_dimension, 2),
                mc_paths=nb_of_paths,
                process_representation=process_representation,
            )
            # one needs two functions for both the coarse and fine process
            c_spot_underlying_stat.density_plot.set_density(
        ...
        if isinstance(control_variates, NoControlVariates):
            ...
            control_variate_statistics = Statistic(
                "CV",
                shape=(len(control_variates.products), payoff_dimension, 2),
                mc_paths=nb_of_paths,
                process_representation=process_representation,
            )

        return MCStatistics(
            payoff_statistics=c_payoff_stat,
            spot_underlying_statistics=c_spot_underlying_stat,
            control_variates_statistics=copy.deepcopy(control_variate_statistics),
        )

    mc_stats = [mc_stat0] + [
        create_statistic(mc_paths) for _ in range(1, initial_level + 1)
    ]
