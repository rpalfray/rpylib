...
class CouplingMarkovChain:
    ...
    def __init__(self, model: LevyModel, method: SamplingMethod, grid: CTMCGrid):
        ...
        self.equivalent_diffusion_coefficient_fine = copy.copy(
            self.fine_process.equivalent_diffusion_coefficient
        )
    ...
    def initialisation(self, product: Product, max_step_epsilon: float = None) -> None:
        ...
        if max_step_epsilon is not None:
            self._path_coupling_simulation = CouplingSimulationMaximumStep(
    ...
    def next_level(
        self,
        mc_paths: int,
        path_managers: [MCPath],
        product: Product,
        max_step_epsilon: float = None,
    ):
        ...
        self.level += 1
        self.grid.refine()
        self.equivalent_diffusion_coefficient_coarse = copy.copy(
            self.equivalent_diffusion_coefficient_fine
        )
        self.fine_process = MarkovChainProcess(
            self.model, method=self.method, grid=self.grid
        )
        ...
        if path_managers is not None:
            ...
            def coupling_deterministic_path(times_input):
                return np.array(
                    [
                        fine_deterministic_path(times_input),
                        coarse_deterministic_path(times_input),
                    ]
                )
...
class CouplingSimulation:
    ...
    def simulate_diffusion_with_coupling(self, sqrt_dts):
        ...
        stddev_fine = (
            sqrt_dts * self.coupling_process.equivalent_diffusion_coefficient_fine
        )
        stddev_coarse = (
            sqrt_dts * self.coupling_process.equivalent_diffusion_coefficient_coarse
        )
    ...
    def coupling_state(self, increment):
        ...
        if not increment % 2:
            ...
            if self.coupling_process.uniform.sample() < self.probability_to_right_jump(
                grid, mass, increment
            ):
...
class CouplingSimulationFixedTimes(CouplingSimulation):
    ...
    def simulate_jumps_with_coupling(self):
        ...
        for k, (slice_fine_states, slice_fine_values) in enumerate(
            zip(fine_states_increments, fines_states_all_values)
        ):
            if slice_fine_states:
                slice_coarse_values = self.coupling_states_for_a_slice(
                    slice_fine_states
                )
...
class CouplingSimulationWithJumpTimes(CouplingSimulation):
    ...
    def simulate_jumps_with_coupling(self):
        ...
        for k, (slice_fine_states, slice_fine_values) in enumerate(
            zip(fine_states_increments, fine_states_all_values)
        ):
            if slice_fine_states:
                slice_coarse_values = self.coupling_states_for_a_slice(
                    slice_fine_states
