...
def create_build_finer_grid_fun(epsilon: float, maturity: float):
    ...
    def _build_finer_grid_default(self, jump_times, fines_states_values, coarse_states_values):
    ...
    def _build_finer_grid(self, jump_times, fines_states_values, coarse_states_values):
        ...
        if not any(dts > epsilon):
            ...
            while positions.size > 0:
                ...
                aug_fine_js = np.insert(aug_fine_js, positions,
                                        np.where(positions == 0, 0, aug_fine_js[..., positions - 1]), axis=-1)
                aug_coarse_js = np.insert(aug_coarse_js, positions,
                                          np.where(positions == 0, 0, aug_coarse_js[..., positions - 1]), axis=-1)
