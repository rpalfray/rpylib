...
class MarkovChainLevyCopula(LevyProcess):
    ...
    def __init__(self, levy_copula_model: LevyCopulaModel, grid: CTMCGrid, method: SamplingMethod):
        ...
        self.sampling = create_sampling_method(model=model_tilde, levy_measure=None,
                                               method=method, grid=grid, is_levy_copula=True,
                                               intensity_of_jumps=self.intensity_of_jumps)

        self._dimension = model_tilde.dimension()
        self._h = grid.h
        self._process_drift = None
        self._path_simulation: MCLevyCopulaSimulation = MCLevyCopulaSimulation(process=self)
    ...
    def initialisation(self, product: Product, max_step_epsilon: float = None) -> None:
        if max_step_epsilon is not None:
            self._path_simulation = MCLevyCopulaSimulationMaximumStep(self, max_step_epsilon)
        ...
        mu_h = np.array([[compute_mu_h(levy_measure=model.levy_triplet.nu, grid=self.grid, axis=self.grid.axes[k],
        ...
        mu_tilde = np.array([[model.levy_triplet.nu.integrate_against_x(-np.inf, -V)
                              + model.levy_triplet.nu.integrate_against_x(V, np.inf) for model in models]]).T
...
class MCLevyCopulaSimulation:
    ...
    def __init__(self, process: MarkovChainLevyCopula):
        ...
        if not model.jump_of_finite_variation():
            ...
            with mp.Pool(processes=nb_of_processes) as pool:
                res = [pool.apply_async(vol_adjustment_ij, args=(i, j, h, model)) for i in range(dimension)
    ...
    def helper_simulate_levy_copula_markov_chain(self, all_nb_of_jumps) -> tuple[list[np.array], list[np.array]]:
        ...
        for k, nb_of_jumps in enumerate(all_nb_of_jumps):
            ...
            xy = (grid[pivot_position + state_increment] for state_increment in states_increments)
...
class MCLevyCopulaSimulationFixedTimes(MCLevyCopulaSimulation, SimulationFixedTimes):
    ...
    def simulate_one_path(self) -> StochasticPath:
        ...
        jumps = np.hstack((np.zeros(self._dimension)[:, np.newaxis], simulated_jumps[:, np.newaxis]))

        # simulate the diffusion part
        simulated_diffusion = self.simulate_diffusion_part()
        diffusion = np.hstack((np.zeros(self._dimension)[:, np.newaxis], simulated_diffusion))
    ...
    def simulate_markov_chain(self) -> MarkovChain:
        ...
        values, all_states_increments = self.helper_simulate_levy_copula_markov_chain(all_nb_of_jumps)
        return MarkovChain(self._times[1:], values, all_states_increments)

    @staticmethod
    def project(values, dim):
        zero = (0.0, )*dim
        definitive_values = (sliceStates[-1] if sliceStates.size else zero for sliceStates in values)
...
class MCLevyCopulaSimulationWithJumpTimes(MCLevyCopulaSimulation, SimulationWithJumpTimes):
    ...
    def simulate_one_path(self) -> StochasticPath:
        ...
        final_jumps = np.zeros(self._dimension) if jump_values.size == 0 else np.array([jp[-1] for jp in jump_values])

        if jump_values.size > 0:
            jumps = np.concatenate((np.zeros(self._dimension)[:, np.newaxis], jump_values, final_jumps[:, np.newaxis]),
                                   axis=1)
        else:
            jumps = np.zeros(shape=(self._dimension, 2))

        simulated_diffusion = self.simulate_diffusion_part(np.sqrt(np.diff(jump_times)))
        diffusion = np.concatenate((np.zeros(self._dimension)[:, np.newaxis], simulated_diffusion), axis=1)
    ...
    def simulate_diffusion_part(self, sqrt_time_increments):
        ...
        brownian_increments = np.random.normal(size=nb).reshape((self._dimension, sqrt_time_increments.size))
        return self.helper_simulate_diffusion_part(sqrt_time_increments, brownian_increments)


class MCLevyCopulaSimulationMaximumStep(MCLevyCopulaSimulationWithJumpTimes, SimulationMaximumStep):
    ...
    def simulate_jumps(self):
        jump_times, jump_values = MCLevyCopulaSimulationWithJumpTimes.simulate_jumps(self)
