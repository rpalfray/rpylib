...
class MarkovChain:
    ...
    def __init__(
        self, times: np.array, values: list[np.array], states_increments: list[np.array]
    ):
        """
...
class MarkovChainProcess(LevyProcess):
    ...
    def __init__(self, model: LevyModel, method: SamplingMethod, grid: CTMCGrid):
        ...
        self.sampling = create_sampling_method(
            model=model_tilde,
            levy_measure=model_tilde.levy_triplet.nu,
            method=method,
            grid=grid,
            is_levy_copula=is_levy_copula,
            intensity_of_jumps=self.intensity_of_jumps,
        )
        vol_adj = vol_adjustment(model_tilde, grid.h)
        self.equivalent_diffusion_coefficient = np.sqrt(
            model_tilde.diffusion_coefficient() ** 2 + vol_adj**2
        )
    ...
    def initialisation(self, product: Product, max_step_epsilon: float = None) -> None:
        if max_step_epsilon is not None:
            self._path_simulation = MCSimulationMaximumStep(
        ...
        mu_tilde = levy_measure.integrate_against_x(
            -np.inf, -v
        ) + levy_measure.integrate_against_x(v, np.inf)
        self._process_drift = (
            self.model.drift() + self.model.levy_triplet.a + mu_tilde - mu_h
        )
...
class MCSimulation:
    ...
    def helper_simulate_markov_chain(
        ...
        for k, nb_of_jumps in enumerate(all_nb_of_jumps):
            ...
            values[k] = np.cumsum(
                [grid[pivot_position + increment] for increment in states_increments]
            )
...
class MCSimulationFixedTimes(MCSimulation, SimulationFixedTimes):
    ...
    def simulate_markov_chain(self) -> MarkovChain:
        ...
        values, all_states_increments = self.helper_simulate_markov_chain(
            self._grid, self._sampling, all_nb_of_jumps
        )
...
class MCSimulationMaximumStep(MCSimulationWithJumpTimes, SimulationMaximumStep):
    ...
    def simulate_jumps(self):
        ...
        if jump_times.size == 0:
            ...
            aug_jump_times, aug_jump_values = self.build_finer_grid(
                jump_times, jump_values
            )
