...
class MarkovChainSDE(Process):
    ...
    def __init__(
        self, model: LevyDrivenSDEModel, method: SamplingMethod, grid: CTMCGrid
    ):
        """
        ...
        if model.dimension_model() == 1:
            self.markov_chain = MarkovChainProcess(
                model=model.driver, method=method, grid=grid
            )
        else:
            self.markov_chain = MarkovChainLevyCopula(
                levy_copula_model=model.driver, grid=grid, method=method
            )

        super().__init__(
            model=model, process_representation=ProcessRepresentation.IDENDITY
        )
...
class MarkovChainLevyLiborModel(MarkovChainSDE):
    ...
    def _integral_zz(self):
        ...
        if dimension == 1:
            res2 = driver.levy_triplet.nu.integrate_against_xx(
                -np.inf, -h / 2
        ...
        for i in range(dimension):
            ...
            nui_x = nui.x_nu
            zz[i, i] = nui.integrate_against_xx(
                -np.inf, -h / 2
            ) + nui.integrate_against_xx(h / 2, np.inf)
            ...
            for j in range(i + 1, dimension):
                ...
                res = scipy.integrate.dblquad(
                    ...
                    a=left_truncation,
                    b=right_truncation,
                    gfun=lambda _: left_truncation,
                    hfun=lambda _: right_truncation,
                    epsabs=epsabs,
                )
                zz[i, j] = zz[j, i] = res[0]
