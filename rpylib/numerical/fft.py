...
class FFTPricer:
    ...
    def _sufficient_condition(self, t: float) -> None:
        ...
        if moment.real > 1e10:
            raise ValueError(
                "sufficient condition on alpha probably not met in the fft pricer"
            )
    ...
    def _psi(self, t: float, v: np.array) -> float:
        ...
        res = (
            np.exp(-self.r * t)
            * self.cf(t=t, x=v - (1 + alpha) * 1j)
            / (alpha * (1 + alpha) - v * v + 1j * (2 * alpha + 1) * v)
        )
        return res
    ...
    def _call_prices(self, maturity: float):
        ...
        w_s = (
            self.eta / 3 * (3.0 + (-1) ** (np.arange(n) + 1) - ds)
        )  # weights for the Simpson's rule
