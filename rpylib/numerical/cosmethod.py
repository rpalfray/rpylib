...
class COSPricer:
    ...
    def __init__(self, model: 'LevyModel', n: int = 10_000, l: int = 10):
        ...
        self.l = l
        self.model = model
        self.cf = model.log_characteristic_function if isinstance(model, ExponentialOfLevyModel) \
    ...
    def _pricing_formula(self, x, time, a, b, vk_coefficients):
        ...
        sum_term = np.sum(np.multiply(phi_s*self._weights*vk_coefficients, exp_s), axis=1)
    ...
    def put(self, strikes, time):
        ...
        return strikes*self._pricing_formula(np.log(spot/strikes), time, a, b, u_values)
