...
class CFLevyModel:
    ...
    def implied_cds_threshold(self, cds_spread: float, recovery_rate: float, h0: float):
        ...
        def fun(threshold):
            return self.cds_spread(level_a=threshold, recovery_rate=recovery_rate) - cds_spread
        a, b = -10, -h0
        res = scipy.optimize.brentq(f=fun, a=a, b=b)
        return res

    def implied_cds_spread(self, pv: float, level_a: float, recovery_rate: float, maturity: float):
        theta = self._theta(level_a)
        r = self.model.r
        ...
        a, b = -5, 10  # `a` is negative because very low number of Monte-Carlo paths might yield to a large variance
