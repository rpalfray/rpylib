...
class CFVarianceGammaModel:
    ...
    def __init__(self, vg_model: 'VarianceGammaModel'):
        if not isinstance(vg_model, rpylib.model.levymodel.purejump.variancegamma.ExponentialOfVarianceGammaModel):
    ...
    def call(self, strike: float, maturity: float):
        ...
        """
        raise NotImplementedError('CFVarianceGammaModel::call something wrong in the implementation, '
    ...
    def _psi_function(a, b, gamma):
        ...
        aux2 = phi(1 + gamma, 1 - gamma, 2 + gamma, 0.5*(1 + u), -sign_a*c*(1 + u))

        term1 = num/(sqrt_2pi*gamma*g_gamma)*sp.special.kv(gamma + 0.5, c)*aux1
        term2 = sign_a*num*(1 + u)/(sqrt_2pi*(1 + gamma)*g_gamma)*sp.special.kv(gamma - 0.5, c)*aux2
        term3 = sign_a*num/(sqrt_2pi*gamma*g_gamma)*sp.special.kv(gamma - 0.5, c)*aux1
