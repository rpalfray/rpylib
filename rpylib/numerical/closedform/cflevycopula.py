...
class CFLevyCopulaModel:
    ...
    def _theta(self, levels_a):
        ...
        if dim > 3:
            raise NotImplementedError('theta function not implemented for dimension > 3')
        if dim != len(levels_a):
            raise ValueError('Expected one level per margin, got: ' + str(len(levels_a)) + 'instead of: '
                             + str(dim))
        ...
        diag = np.array([model.mass(*interval_I(ai)) for model, ai in zip(self.levy_copula_model.models, levels_a)])
    ...
    def implied_cds_spread(self, pv: float, level_a: list[float], recovery_rate: float, maturity: float):
        theta = self._theta(level_a)
        r = self.levy_copula_model.models[0].r
        default_leg = (1 - recovery_rate)*(1 - np.exp(-(r+theta)*maturity))*theta/(r+theta)
        fixed_leg = (1 - np.exp(-(r+theta)*maturity))/(r+theta)

        def fun(spread):
            return default_leg - spread*fixed_leg - pv
        a, b = -10, 10  # `a` is negative because very low number of Monte-Carlo paths might yield to a large variance
