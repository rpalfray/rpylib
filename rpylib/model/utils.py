...
from .levymodel.purejump.variancegamma import VGParameters, VarianceGammaModel, ExponentialOfVarianceGammaModel
from ..distribution.levycopula import LevyCopula, ClaytonCopula, IndependentComponentsCopula, DependentComponentsCopula
...
models_description = {
    ModelType.BLACKSCHOLES: ModelClasses(BlackScholesParameters, PureDiffusiveModel, BlackScholesModel),
    ModelType.HEM: ModelClasses(HEMParameters, HEMModel, ExponentialOfHEMModel),
    ModelType.MERTON: ModelClasses(MertonParameters, MertonModel, ExponentialOfMertonModel),
    ModelType.CGMY: ModelClasses(CGMYParameters, CGMYModel, ExponentialOfCGMYModel),
    ModelType.VG: ModelClasses(VGParameters, VarianceGammaModel, ExponentialOfVarianceGammaModel),
}
...
def _create_hem_model(sigma: float = 0.05, p: float = 0.6, eta1: float = 20, eta2: float = 25, intensity: float = 3):
    return helper_model(ModelType.HEM, False)(sigma=sigma, p=p, eta1=eta1, eta2=eta2, intensity=intensity)


def _create_merton_model(sigma: float = 0.05, sigma_j: float = 0.05, mu_j: float = 0.01, intensity: float = 3):
    return helper_model(ModelType.MERTON, False)(sigma=sigma, sigma_j=sigma_j, mu_j=mu_j, intensity=intensity)


def _create_cgmy_model(c: float = 1., g: float = 15, m: float = 20, y: float = 0.5):
    return helper_model(ModelType.CGMY, False)(c=c, g=g, m=m, y=y)


def _create_variancegamma_model(sigma: float = 0.1, nu: float = 0.06, theta: float = 0.1):
    return helper_model(ModelType.VG, False)(sigma=sigma, nu=nu, theta=theta)


def _create_blackscholes_model(spot: float = 100, r: float = 0.02, d: float = 0., sigma: float = 0.10):
    return helper_model(ModelType.BLACKSCHOLES)(spot=spot, r=r, d=d, sigma=sigma)


def _create_exp_hem_model(spot: float = 100, r: float = 0.02, d: float = 0., sigma: float = 0.05, p: float = 0.6,
                          eta1: float = 20, eta2: float = 25, intensity: float = 3):
    return helper_model(ModelType.HEM)(spot=spot, r=r, d=d, sigma=sigma, p=p, eta1=eta1, eta2=eta2, intensity=intensity)
...
def _create_exp_cgmy_model(spot: float = 100, r: float = 0.02, d: float = 0., c: float = 1., g: float = 15,
                           m: float = 20, y: float = 0.5):
    return helper_model(ModelType.CGMY)(spot=spot, r=r, d=d, c=c, g=g, m=m, y=y)


def _create_exp_variancegamma_model(spot: float = 100, r: float = 0.02, d: float = 0., sigma: float = 0.1,
                                    nu: float = 0.06, theta: float = 0.1):
    return helper_model(ModelType.VG)(spot=spot, r=r, d=d, sigma=sigma, nu=nu, theta=theta)
...
models_creators = {
    ...
    ModelType.VG: ModelCreators(_create_variancegamma_model, _create_exp_variancegamma_model),
}
...
def create_levy_model(model_type: ModelType) -> LevyModelFunctor:
    ...
    if model_type not in models_creators:
        raise NotImplementedError('No implemented helper function for this Lévy model yet')

    return models_creators[model_type].LevyModel


def create_exponential_of_levy_model(model_type: ModelType) -> ExponentialOfLevyModelFunctor:
    ...
    """
    if model_type not in models_creators:
        raise NotImplementedError('No implemented helper function for this exponential of Lévy model yet')
...
def calibrate_model_parameter(model: ExponentialOfLevyModel, parameter: str, parameter_interval: tuple,
                              product: Product, market_price: float) -> float:
    ...
    try:
        ...
        raise ValueError('Parameter cannot be calibrated given the market data and its range constraints')

    return res


def calibrate_model_parameter_to_atm_call(model: ExponentialOfLevyModel, parameter: str, parameter_interval: tuple,
                                          maturity: float, bs_sigma: float = 0.10) -> float:
    ...
    call = Product(payoff_underlying=Spot(), payoff=Vanilla(strike=strike, payoff_type=PayoffType.CALL),
                   maturity=maturity)
    bs_model = create_exponential_of_levy_model(ModelType.BLACKSCHOLES)(spot=model.spot, r=model.r, d=model.d,
                                                                        sigma=bs_sigma)
    bs_price = CFBlackScholes(bs_model).call(strike=model.spot, maturity=maturity)
    return calibrate_model_parameter(model=model, parameter=parameter, parameter_interval=parameter_interval,
                                     product=call, market_price=bs_price)


DefaultCalibrationConfiguration = namedtuple('DefaultCalibrationConfiguration', 'parameter parameter_interval')
default_calibration = {
    ModelType.HEM: DefaultCalibrationConfiguration('sigma', (0., 1.)),
    ModelType.MERTON: DefaultCalibrationConfiguration('mu_j', (0., 1.)),
    ModelType.CGMY: DefaultCalibrationConfiguration('c', (1e-12, 20.)),
    ModelType.VG: DefaultCalibrationConfiguration('sigma', (0.00001, 1.)),
}


def run_default_calibration(model: ExponentialOfLevyModel, maturity: float, bs_sigma: float = 0.10):
    ...
    calibrated_parameter = calibrate_model_parameter_to_atm_call(model=model, parameter=def_calibration.parameter,
                                                                 parameter_interval=def_calibration.parameter_interval,
                                                                 maturity=maturity, bs_sigma=bs_sigma)
...
def create_levy_copula_model(models: [LevyModel], copula: LevyCopula) -> LevyCopulaModel:
...
def create_levy_forward_market_model_copula(driver: [LevyModel]) -> LevyForwardModel:
    ...
    return LevyForwardModel(driver=levy_copula, ois_rates=ois_rates, tenors=tenors, sigma=sigma)
