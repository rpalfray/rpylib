...
class LevyForwardModel(LevyDrivenSDEModel):
    ...
    def __init__(
        self,
        ois_rates: Union[float, np.array],
        tenors: list[float],
        sigma: np.array,
        driver: LevyDriver,
    ):
        super().__init__(
            driver=driver,
            x0=np.array(ois_rates),
            a=ForwardMarketSDEFunction(sigma=sigma, tenors=tenors),
        )
        ...
        if not driver.finite_first_moment():
            raise ValueError(
                "In the Lévy model, the driver must have a finite first moment."
            )
