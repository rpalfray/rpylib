...
class LevyLiborModel(LevyDrivenSDEModel):
    ...
    """
    def __init__(self, libor_rates: Union[float, np.array], tenors: list[float], sigma: np.array, driver: LevyDriver):
        super().__init__(driver=driver, x0=np.array(libor_rates), a=LiborSDEFunction(sigma=sigma, tenors=tenors))
        self.tenors = np.array(sorted(tenors))
        self.deltas = np.diff(tenors).astype(float)

        # consistency checks:
        if not driver.finite_first_moment():
            raise ValueError('In the Lévy Libor model, the driver must have a finite first moment.')
