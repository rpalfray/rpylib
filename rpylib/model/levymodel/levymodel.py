...
class LevyTriplet:
    ...
    def __init__(self, sigma: float, nu: LevyMeasure, a: float = 0,
        ...
        self._drift_mapping = {LevyRepresentation.ONEONE: self.canonical_drift,
    ...
    def canonical_drift(self) -> float:
        ...
        if self.representation == LevyRepresentation.ZERO or \
                (self.representation == LevyRepresentation.TILDE and self.nu.jump_of_finite_variation()):
...
class LevyModel(Model):
    ...
    def __init__(self, model_type: ModelType, levy_triplet: LevyTriplet, cumulant: Cumulant):
        ...
        self._original_drift = levy_triplet.a  # needed if the Lévy representation is changed

    def __str__(self):
        cls = self.__class__.__name__
        return '{cls} with Lévy triplet: {levy_triplet}'.format(cls=cls, levy_triplet=str(self.levy_triplet))
    ...
    def jump_increment(self, n) -> np.array:
        ...
        """
        raise ValueError('jump increments cannot be simulated in closed-form for this model')
