...
class CGMYParameters(Parameters):
    ...
    def __repr__(self) -> str:
        return 'CGMYParameters(c={c}, g={g}, m={m}, y={y})'.format(c=self.c, g=self.g, m=self.m, y=self.y)
...
class _CGMYCumulant(Cumulant):
    ...
    def cumulant1(self, t: float) -> float:
        drift = self.drift
        c, g, m, y = self.parameters.c, self.parameters.g, self.parameters.m, self.parameters.y
        if y == 1:
...
class _CGMYLevyMeasure(LevyMeasure):
    ...
    def integrate_against_x(self, a: float, b: float) -> float:
        ...
        if b <= 0:
            if a == -np.inf:
                ...
                return -c*(self.__integrate_h_to_inf_for_xx(alpha=y, h=-a, u=g) -
    ...
    def __integrate_h_to_inf(self, alpha, h, u):
        ...
        expmuh = np.exp(-uh)
        if alpha >= 1:
            return expmuh/(alpha*h**alpha) - (u/alpha)*self.__integrate_h_to_inf(alpha=alpha - 1, h=h, u=u)
        
        g2malpha = scipy.special.gamma(2-alpha)
        ginccuh = scipy.special.gammaincc(2-alpha, uh)
        
        res = (expmuh*(1 + uh/(1-alpha)) - (uh**alpha)*g2malpha*ginccuh/(1-alpha))/(alpha*(h**alpha))
    ...
    def __integrate_levy_measure_a_to_b(self, a, b):
        if a >= 0 and b > 0:
            return self.__integrate_levy_measure_a_to_inf(a) - self.__integrate_levy_measure_a_to_inf(b)
        elif a < 0 and b <= 0:
            return self.__integrate_levy_measure_inf_to_b(b) - self.__integrate_levy_measure_inf_to_b(a)
        else:  # a < 0 < b
            if self.parameters.y > 0:
                return np.inf
            else:
                return self.__integrate_levy_measure_a_to_b(a, 0.0) + self.__integrate_levy_measure_a_to_b(0.0, b)
    ...
    def __integrate_h_to_inf_for_xx(alpha, h, u):
        ...
        if alpha == 1.0:
            ...
            g2malpha = scipy.special.gamma(2-alpha)
            ginccuh = scipy.special.gammaincc(2-alpha, uh)
            res = (h**(1-alpha)*expmuh - u**(alpha-1)*g2malpha*ginccuh)/(alpha-1)
...
class CGMYModel(LevyModel):
    ...
    def __init__(self, parameters: CGMYParameters):
        ...
        triplet = LevyTriplet(a=0, sigma=0.0, nu=_CGMYLevyMeasure(parameters), representation=representation)
        super().__init__(model_type=ModelType.CGMY, levy_triplet=triplet, cumulant=cumulant)

    def __repr__(self):
        return 'CGMYModel(parameters={parameters})'.format(parameters=repr(self.parameters))
    ...
    def levy_exponent_pure_jump(self, x: complex) -> complex:
        ...
        if y == 0:
            ...
            res += c*((g + x)*np.log(g + x) - g*np.log(g) + (m - x)*np.log(m - x) - m*np.log(m))
        else:
            ...
            aux_g = x*y*g**(y-1)
            aux_m = x*y*m**(y-1)
            res += p._CGammamY*(np.power(g + x, y) - aux_g + np.power(m - x, y) + aux_m - p._GpowerY - p._MpowerY)
