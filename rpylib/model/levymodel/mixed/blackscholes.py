...
class PureDiffusiveModel(LevyModel):
    ...
    def __init__(self, mu: float, sigma: float):
        levy_triplet = LevyTriplet(a=mu, sigma=sigma, nu=_LevyMeasureDiffusion(),
                                   representation=LevyRepresentation.ZERO)
        cumulant = _BlackScholesCumulant(drift=mu, parameters=BlackScholesParameters(sigma=sigma))
        super().__init__(model_type=ModelType.BLACKSCHOLES, levy_triplet=levy_triplet, cumulant=cumulant)

    def __repr__(self):
        return 'PureDiffusiveModel(mu={mu}, sigma={sigma})'\
            .format(mu=self.levy_triplet.a, sigma=self.levy_triplet.sigma)
...
class BlackScholesModel(ExponentialOfLevyModel):
    """Black-Scholes model"""
    def __init__(self, spot: float, r: float, d: float, parameters: BlackScholesParameters):
