...
class HEMParameters(Parameters):
    ...
    def __init__(self, sigma: float, p: float, eta1: float, eta2: float, intensity: float):
        self.sigma = sigma
        self.p = p
        self.eta1 = eta1
        self.eta2 = eta2
    ...
    def __repr__(self) -> str:
        return 'HEMParameters(sigma={sigma}, eta1={eta1}, eta2={eta2}, intensity={intensity})'\
            .format(sigma=self.sigma, eta1=self.eta1, eta2=self.eta2, intensity=self.intensity)

    def initialisation(self):
        self._xi = self.p*self.eta1/(self.eta1-1) + (1-self.p)*self.eta2/(self.eta2+1) - 1
...
class _HEMLevyMeasure(LevyMeasure):
    ...
    def integrate(self, a: float, b: float) -> float:
        ...
        if a > b:
            raise ValueError('expected a<=b when integrating the levy measure of the HEM model')
    ...
    def integrate_against_x(self, a: float, b: float) -> float:
        ...
        if b <= 0:
            if a == -np.inf:
                ...
                return intensity*(1-p)*(np.exp(eta2*b)*(b - 1/eta2) - np.exp(eta2*a)*(a - 1/eta2))
        elif a >= 0:
            if b == np.inf:
                return intensity*p*np.exp(-eta1*a)*(a + 1/eta1)
            else:
                return -intensity*p*(np.exp(-eta1*b)*(b + 1/eta1) - np.exp(-eta1*a)*(a + 1/eta1))
        else:
    ...
    def integrate_against_xx(self, a: float, b: float) -> float:
        ...
        if b <= 0:
            ...
            if a == -np.inf:
                ...
                a_eta2 = a*eta2
                return intensity*(1-p)*(b_term - np.exp(a_eta2)*(a_eta2*(a_eta2 - 2) + 2))/eta2**2
        elif a >= 0:
            a_eta1 = a*eta1
            ...
            if b == np.inf:
                ...
                b_eta1 = b*eta1
                return intensity*p*(a_term + np.exp(-b_eta1)*(-b_eta1*(b_eta1 + 2) - 2))/eta1**2
        else:
...
class HEMModel(LevyModel):
    ...
    def __init__(self, parameters: HEMParameters):
        self.parameters = parameters
        a = -parameters.intensity*(parameters.p/parameters.eta1 - (1-parameters.p)/parameters.eta2)
        cumulant = _HEMCumulant(drift=a, parameters=parameters)
        levy_triplet = LevyTriplet(a=a, sigma=parameters.sigma, nu=_HEMLevyMeasure(parameters),
                                   representation=LevyRepresentation.ZERO)

        super().__init__(model_type=ModelType.HEM, levy_triplet=levy_triplet, cumulant=cumulant)

    def __repr__(self) -> str:
        return 'HEMModel(parameters={parameters})'.format(parameters=repr(self.parameters))
