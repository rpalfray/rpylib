...
class MertonParameters(Parameters):
    ...
    def __repr__(self) -> str:
        return 'MertonParameters(sigma={sigma}, mu_j={mu_j}, sigma_j={sigma_j}, intensity={intensity})'\
            .format(sigma=self.sigma, mu_j=self.mu_j, sigma_j=self.sigma_j, intensity=self.intensity)
...
class _MertonLevyMeasure(LevyMeasure):
    ...
    def integrate(self, a: float, b: float) -> float:
        mu_j, sigma_j, intensity = self.parameters.mu_j, self.parameters.sigma_j, self.parameters.intensity
        return 0.5*intensity*(self._helper_erf_aux(mu_j, sigma_j, b) - self._helper_erf_aux(mu_j, sigma_j, a))

    def integrate_against_x(self, a: float, b: float) -> float:
        mu_j, sigma_j, intensity = self.parameters.mu_j, self.parameters.sigma_j, self.parameters.intensity

        def fun_aux(x):
            return 0.5*mu_j*self._helper_erf_aux(mu_j, sigma_j, x) \
                   - sigma_j/(np.sqrt(2*np.pi))*np.exp(-(x-mu_j)**2/(2*sigma_j**2))
    ...
    def integrate_against_xx(self, a: float, b: float) -> float:
        ...
        def fun_aux(x):
            first_term = 0.5*(mu_j**2 + sigma_j**2)*self._helper_erf_aux(mu_j, sigma_j, x)
            if x == np.inf or x == -np.inf:
                return first_term
            else:
                return first_term - sigma_j/(np.sqrt(2*np.pi))*(mu_j + x)*np.exp(-(x-mu_j)**2/(2*sigma_j**2))
...
class _MertonCumulant(Cumulant):
    ...
    def cumulant4(self, t: float) -> float:
        params = self.parameters
        mu_j, sigma_j, intensity = params.mu_j, params.sigma_j, params.intensity
        return intensity*(mu_j**4 + 3*sigma_j**4 + 6*mu_j**2 * sigma_j**2)*t

    def cumulant6(self, t: float) -> float:
        params = self.parameters
        mu_j, sigma_j, intensity = params.mu_j, params.sigma_j, params.intensity
        return intensity*(45*sigma_j**4 * mu_j**2 + 15*sigma_j**2 * mu_j**4 + mu_j**6 + 15*sigma_j**6)*t
...
class MertonModel(LevyModel):
    ...
    def __init__(self, parameters: MertonParameters):
        ...
        levy_triplet = LevyTriplet(a=a, sigma=parameters.sigma, nu=_MertonLevyMeasure(parameters),
                                   representation=LevyRepresentation.ZERO)
        super().__init__(model_type=ModelType.MERTON, levy_triplet=levy_triplet, cumulant=cumulant)

    def __repr__(self) -> str:
        return 'MertonModel(parameters={parameters})'.format(parameters=repr(self.parameters))
    ...
    def jump_increment(self, n) -> np.array:
        z = np.random.normal(loc=self.parameters.mu_j, scale=self.parameters.sigma_j, size=n)
        return z
...
class ExponentialOfMertonModel(ExponentialOfLevyModel):
    ...
    def __init__(self, spot: float, r: float, d: float, parameters: MertonParameters):
        ...
        sigma, mu_j, sigma_j, intensity = parameters.sigma, parameters.mu_j, parameters.sigma_j, parameters.intensity
        self._process_drift = r - d - 0.5*sigma**2 - intensity*(np.exp(mu_j + 0.5*sigma_j**2) - 1)
