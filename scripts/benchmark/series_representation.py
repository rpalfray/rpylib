...
def pricing(configuration, process, product):
    ...
    price, mc_stddev = res.price(no_control_variates=True), res.mc_stddev(
...
@timer
def benchmark_series_representation(
    strikes: np.array,
    mc_paths: int,
    tau: float,
    ...
    maturity = 0.02
    asian_options = Product(
        payoff_underlying=Mean(),
        payoff=Vanilla(
            strike=strikes * 0.5 * (spot1 + spot2), payoff_type=PayoffType.CALL
        ),
        maturity=maturity,
    )
    bestof_options = Product(
        payoff_underlying=MaximumOfPerformances([spot1, spot2]),
        payoff=Vanilla(strike=strikes, payoff_type=PayoffType.CALL),
        maturity=maturity,
    )
    ...
    copula_std, copula_wtd = ClaytonCopula(theta=10, eta=0.75), ClaytonCopula(
        theta=0.61, eta=0.99
    )
    ...
    call1 = Product(
        ...
        payoff=Vanilla(strike=spot1 * strikes, payoff_type=PayoffType.CALL),
        maturity=maturity,
    )
    call2 = Product(
        payoff_underlying=NthSpot(2),
        payoff=Vanilla(strike=spot2 * strikes, payoff_type=PayoffType.CALL),
        maturity=maturity,
    )
    price_calls = [
        COSPricer(model1).price(product=call1),
        COSPricer(model2).price(product=call2),
    ]
    ...
    results_ctmc = [
        pricing(
            configuration,
            MarkovChainLevyCopula(model, ctmc_grid_function(model), method),
            options,
        )
        for _, model in levy_copula_models.items()
        for options in all_options
    ]

    results_series = [
        pricing(
            configuration, LevyCopula2dSeriesRepresentation(model, tau=tau), options
        )
        for _, model in levy_copula_models.items()
        for options in all_options
    ]

    save_benchmark_series_results(
        strikes=strikes,
        levy_copula_models=levy_copula_models,
        all_options=all_options,
        results_ctmc=results_ctmc,
...
def create_result_files(
    strikes: list[float], products: list[str], result_folder: str, clear_file=False
) -> None:
...
def dump_results(
    result_directory: str, process_type, copula_type: str, product_type: str, results
):
...
def save_benchmark_series_results(
    strikes,
    levy_copula_models,
    all_options,
    results_ctmc,
    ...
    result_folder_name_val = result_folder_name or os.path.join(
        os.getcwd(), "results/series"
    )
    create_folder(folder_name=result_folder_name_val)
    create_result_files(
        strikes=strikes,
        products=["asian", "bestof"],
        result_folder=result_folder_name_val,
        clear_file=True,
    )
    ...
    for i, key in enumerate(levy_copula_models):
        for j, _ in enumerate(all_options):
            ...
            dump_results(
                result_directory=result_folder_name_val,
                process_type="ctmc",
                copula_type=key,
                product_type=product_type,
                results=results_ctmc[2 * i + j],
            )
            dump_results(
                result_directory=result_folder_name_val,
                process_type="series",
