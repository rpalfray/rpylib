...
def cds_spread(
    ...
    ctmc_grid_function: Callable[[float, list[float]], CTMCGrid],
    ...
    survival_probabilities = [
        cf_pricer.survival_probability(level_a=level_a, t=maturity)
        for level_a in levels_as
    ]
    print(
        "levels a:               ",
        ", ".join("{:>8.4f}".format(level_a) for level_a in levels_as),
    )
    print(
        "survival probabilities: ",
        ", ".join("{:>7.2f}%".format(100 * p) for p in survival_probabilities),
    )

    configuration = ConfigurationStandard(mc_paths=mc_paths)

    def pricing_function(spread, levels_a):
        process = MarkovChainProcess(
            model=model,
            method=SamplingMethod.INVERSION,
            grid=ctmc_grid_function(h0, levels_a),
        )
        mc_engine = Engine(configuration=configuration, process=process)

        product = Product(
            payoff_underlying=DefaultTime(default_level=levels_a),
            payoff=CDS(
    ...
    results = [
        pricing_function(spread / 10_000, level_a)
        for spread, level_a in zip(cds_spreads_bps, levels_as)
    ]
    ...
    for (
        ...
        print(
            "spread: {:6.1f}(bps), implied spread: {:6.2f}(bps), stddev [{:>5.2f}/{:>5.2f}](bps), price: {:5.4f}, "
            "mc_stddev: {:5.4f}".format(
                spread,
                implied_spread,
                spread_stddev_p,
                spread_stddev_m,
    ...
    save_benchmark_single_name_cds_results(
        name,
        cds_spreads_bps,
        levels_as,
        survival_probabilities,
...
def helper_extract_result(
    notional, cf_pricer, recovery_rate, maturity, cds_spreads_bps, results, levels_as
):
    ...
    for spread, (pv, pv_stddev), level_a in zip(cds_spreads_bps, results, levels_as):
        implied_spread = cf_pricer.implied_cds_spread(
            pv=pv / notional,
            level_a=level_a,
            recovery_rate=recovery_rate,
            maturity=maturity,
        )
        pstddev = cf_pricer.implied_cds_spread(
            pv=(pv + z_value * pv_stddev) / notional,
            level_a=level_a,
            recovery_rate=recovery_rate,
            maturity=maturity,
        )
        mstddev = cf_pricer.implied_cds_spread(
            pv=(pv - z_value * pv_stddev) / notional,
            level_a=level_a,
            recovery_rate=recovery_rate,
            maturity=maturity,
...
def save_benchmark_single_name_cds_results(
    name: str,
    cds_spreads_bps,
    levels_as,
    survival_probabilities,
...
if __name__ == "__main__":
    ...
    def my_ctmc_grid_function(my_h, my_levels_a):
        return CTMCCredit(
            h=my_h, level_a=my_levels_a, model=my_model
        )  # this version will be much faster without
