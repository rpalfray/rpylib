...
def statistic_standard(
    ...
    # Control Variate
    forward = Product(
        payoff_underlying=Spot(), payoff=Forward(strike=strikes), maturity=maturity
    )
    control_variate = ControlVariates(
        [forward], [COSPricer(model).price(product=forward)]
    )
    ...
    print(
        "strikes:           ",
        "* ".join("{:>6.2f} ".format(strike) for strike in strikes),
    )
    print("theoretical price: ", "* ".join("{:>6.3f} ".format(p) for p in price))
    print(
        "mc price         : ",
        "* ".join("{:>6.3f} ".format(p) for p in np.array(mc_price, ndmin=1)),
    )
    print(
        "mc price / no CV : ",
        "* ".join("{:>6.3f} ".format(p) for p in np.array(mc_price_no_cv, ndmin=1)),
    )
    print(
        "mc stddev        : ",
        "* ".join("{:>6.3f} ".format(p) for p in np.array(mc_stddev, ndmin=1)),
    )
    print(
        "mc stddev / no CV: ",
        "* ".join("{:>6.3f} ".format(p) for p in np.array(mc_stddev_no_cv, ndmin=1)),
    )
