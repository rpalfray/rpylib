...
def convergence_rate(model: LevyModel, product: Product, max_level: int, mc_paths: int, method: SamplingMethod,
                     grid: CTMCGrid):
...
@timer
def coupling_variances(name, model, mc_paths: int, max_level: int, maturity: float, h0: float,
    ...
    result_folder_name = result_folder_name or Path(Path(__file__).cwd().parent, 'results/giles_convergence/1d')
