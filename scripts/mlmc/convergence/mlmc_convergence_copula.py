...
def coupling_variances_copula(
    ...
    results = convergence_rate_copula(
        levy_copula_model, product, max_level, mc_paths, method, grid
    )
    consistency_check = results.mlmc_results.consistency_check

    result_folder_name = result_folder_name or Path(
        Path(__file__).cwd().parent, "results/giles_convergence/copulas"
    )
