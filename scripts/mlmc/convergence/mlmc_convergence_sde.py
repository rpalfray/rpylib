...
def convergence_rate(model: LevyForwardModel, product: Product, max_level: int, mc_paths: int, method: SamplingMethod,
                     grid: CTMCGrid):
    ...
    configuration = ConfigurationMultiLevel(convergence_rates=cr, convergence_criteria=convergence_criteria,
                                            initial_level=initial_level, maximum_level=maximum_level,
                                            initial_mc_paths=initial_mc_paths)  # nb_of_processes=1
...
@timer
def coupling_variances_sde(name: str, model, mc_paths: int, h0: float, max_level: int, result_folder_name: str = None):
    product = Product(payoff_underlying=Libors(), payoff=Swaption(underlying_rates=model.x0, deltas=model.deltas,
                                                                  strike=np.average(model.x0)),
                      maturity=model.tenors[0], notional=100.0)
    ...
    result_folder_name = result_folder_name or Path(Path(__file__).cwd().parent, 'results/giles_convergence/sde')
...
if __name__ == '__main__':
    ...
    coupling_variances_sde(my_name, my_sde_driven_model, my_mc_paths, my_h0, my_max_level)
