...
def polynomial_fit(figaxis, xaxis, yaxis, levels, startIndex, curvelabel):
    ...
    print(
        "fitted curve for {:12}: ".format(curvelabel)
        + "{a:0.3}x + {b:0.3}".format(a=a, b=b)
    )
    return a, b
...
def plot_graphs(
    model, sFitIndex, h0, mlevell, vlevell, consistencycheck, kurtosis, ml, vl, cl
):
    ...
    a1, b1 = polynomial_fit(
        axvl, log_hl[1:], log2_vl, levels[1:], sFitIndex, "variances"
    )
    new_convergence_rate = a1

    axkurt.plot(
        levels[1:], kurtosis[1:], color="b", label="kurtosis", marker="*", linewidth=1.0
    )
    axcc.plot(
        levels[1:],
        consistencycheck[1:],
    ...
    a_cl, b_cl = polynomial_fit(
        axcl, log_hl[1:], log2_cl[1:], levels[1:], sFitIndex, "cost cl"
    )
    cl_convergence_rate = a_cl

    avgcost = sum(log2_cl[-4:]) / len(log2_cl[-4:]) - sum(
        [np.log2(h ** (-bg)) for h in hl[-4:]]
    ) / len(hl[-4:])
    axcl.plot(
        levels,
        avgcost + np.array([np.log2(h ** (-bg)) for h in hl]),
        color="r",
        label="th cost l (slope)",
        marker="*",
        linewidth=1.0,
    )
