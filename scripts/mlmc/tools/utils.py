...
levy_models = {'hem': create_levy_model(ModelType.HEM)(),
...
exp_of_levy_models = {
    ...
    'cgmy02': create_exponential_of_levy_model(ModelType.CGMY)(c=0.7, g=15, m=20, y=0.2),
    'cgmy04': create_exponential_of_levy_model(ModelType.CGMY)(c=0.7, g=15, m=20, y=0.4),
    'cgmy12': create_exponential_of_levy_model(ModelType.CGMY)(c=0.019, g=2, m=4, y=1.2),
    'cgmy15': create_exponential_of_levy_model(ModelType.CGMY)(c=0.007, g=2, m=4, y=1.5),
}

calibrated_models = {key: run_default_calibration(model=val, maturity=cases_data['maturity'])
...
def helper_data(name: str, model_case: Literal['1d', 'copulas', 'sde'], root_path: Path) -> tuple[
    CTMCGrid, Union[LevyModel, LevyCopulaModel, LevyDrivenSDEModel], Product]:
    # retrieve the data generated by the `mlmc_convergence_1d` script
...
def save_mlmc_coupling_variances_results(file_path, results, consistency_check, beta):
    with open(file_path, 'w', newline='') as f:
        writer = csv.writer(f, delimiter=',', quoting=csv.QUOTE_NONE, escapechar=' ')
        writer.writerow('level meanlevell varlevell consistencycheck kurtosis ml vl cl vl_slope ml_slope'.split())
        res_mlmc = results.mlmc_results
        levels = list(range(len(res_mlmc.mean_level_l)))
        rows = zip(levels, res_mlmc.mean_level_l, res_mlmc.var_level_l, consistency_check, res_mlmc.kurtosis,
                   res_mlmc.ml, res_mlmc.vl, res_mlmc.cl)

        mean_logvl = sum(np.log2(res_mlmc.vl))/len(res_mlmc.vl)
        mean_logml = sum(np.log2(res_mlmc.ml))/len(res_mlmc.ml)
        mean_levels = sum(levels)/len(levels)

        for k, row in enumerate(rows):
            level = levels[k]
            slope_term_vl = 2**(-(2-beta) * level + (mean_logvl + (2-beta) * mean_levels))
            slope_term_ml = 2**(-(1-beta/2) * level + (mean_logml + (1-beta/2) * mean_levels))
            to_write = row + (slope_term_vl, slope_term_ml)
            writer.writerow(to_write)


def save_mlmc_coupling_convergence_results(rmses, outputs, root_path_results, file_result_name, beta):
    ...
    normalised_costs_std_mc = [rmse * rmse * cost_cLs[j] * Nl0s[j] for j, rmse in enumerate(rmses)]
    with open(path_costs, 'a+', newline=''):
        try:
            df1 = pandas.read_csv(path_costs)
        except pd.errors.EmptyDataError:
            df1 = pd.DataFrame(columns=['rmse', 'costxrmse2', 'costxrmse2stdmc', 'slope'], dtype=float)
        for rmse, cost, cost_std in zip(rmses, normalised_costs, normalised_costs_std_mc):
            to_add = pd.DataFrame({'rmse': [rmse], 'costxrmse2': [cost], 'costxrmse2stdmc': [cost_std], 'slope': [0.0]})
