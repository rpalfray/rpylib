...
def giles_variances_graphs_copulas(model_str: str, max_level: int, mc_paths: int, h0: float):
    ...
    coupling_variances_copula(name=model_str, levy_copula_model=model, mc_paths=mc_paths, max_level=max_level,
                              maturity=my_maturity, h0=h0, result_folder_name=result_folder_name)


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Generate data for weak/strong convergence graphs as in Giles '
                                                 'for Levy copulas')
    ...
    giles_variances_graphs_copulas(model_str=args.model, max_level=args.max_level, mc_paths=args.mc_paths, h0=args.h0)
